...
def analyze_dataset_and_roi(dataset, file, roi):
    ...
    for chol_trajectory in trajectories_by_label[CHOL_NOMENCLATURE]:
        ...
        for chol_confinement in chol_confinements:
            for btx_confinement in btx_confinements:
                if np.linalg.norm(chol_confinement.centroid-btx_confinement.centroid) < 1 and both_trajectories_intersect(chol_confinement, btx_confinement, via='hull'):#um
                    chol_trajectory.info[f'number_of_confinement_zones_with_{BTX_NOMENCLATURE}'] += 1
                    break
    ...
    for btx_trajectory in trajectories_by_label[BTX_NOMENCLATURE]:
        ...
        for btx_confinement in btx_confinements:
            for chol_confinement in chol_confinements:
                if np.linalg.norm(chol_confinement.centroid-btx_confinement.centroid) < 1 and both_trajectories_intersect(chol_confinement, btx_confinement, via='hull'):#um
                    btx_trajectory.info[f'number_of_confinement_zones_with_{CHOL_NOMENCLATURE}'] += 1
                    break
...
for file_and_roi in tqdm.tqdm(file_and_rois):
    analyze_dataset_and_roi(file_and_roi[0], file_and_roi[1], file_and_roi[2])
