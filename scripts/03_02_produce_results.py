...
for index, dataset in enumerate(new_datasets_list):
    print(dataset,index)
    if index < len(INDIVIDUAL_DATASETS):
        continue
...
basic_info_file.write(f"{BTX_NOMENCLATURE} -> Fraction: {np.mean(fractions)}, S.E.M: {sem(fractions)}s\n")
...
basic_info_file.write(f"{CHOL_NOMENCLATURE} -> Fraction: {np.mean(fractions)}, S.E.M: {sem(fractions)}s\n")
