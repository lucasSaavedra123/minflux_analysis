...
from utils import measure_overlap, extract_dataset_file_roi_file
...
for dataset, file, roi in file_and_rois:
    ...
    simulated_values = [get_random_value([t.copy() for t in trajectories]) for i in tqdm.tqdm(range(99))]
    simulated_values.append(real_value)
    np.savetxt(f'{dataset}_{file}_{roi}.txt', simulated_values)
    #plt.hist(simulated_values)
    #plt.xlim([0,1])
    #plt.axvline(real_value)
    #plt.tight_layout()
    #plt.show()
    #exit()
